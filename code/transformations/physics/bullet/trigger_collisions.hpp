  ...
  */
  ...
  Data::data_lock(trigger_data);
  {
    ...
    for(size_t t = 0; t < trigger_count; ++t)
    {
      ...
      if(!ghost_object)
      {
        LOG_ERROR_ONCE("Trigger Logic err");
        continue;
      }
      
      const btBroadphasePairArray &pair_array(ghost_object->getOverlappingPairCache()->getOverlappingPairArray());
      
      const int pair_size(pair_array.size());
      
      for(int p = 0; p < pair_size; ++p)
      {
        const btBroadphasePair &pair = pair_array[p];
        
        btBroadphasePair *collision_pair = dynamicsWorld->getPairCache()->findPair(pair.m_pProxy0, pair.m_pProxy1);
        ...
        {
          btPersistentManifold *manifold = manifold_array[m];
