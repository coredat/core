                   ...
                   const bool is_dynamic)
{
  ...
  glBufferData(GL_ARRAY_BUFFER, size_of_vbo, vbo_buffer, is_dynamic ? GL_DYNAMIC_DRAW : GL_STATIC_DRAW);
  glBindBuffer(GL_ARRAY_BUFFER, 0);

  if(out_vbo->vertex_buffer_id)
  {
    out_vbo->number_of_entries = static_cast<uint32_t>(number_of_elements);
  }
