  ...
  *data_memory = Memory::request_memory_chunk(bytes_to_alloc, "material_data");
  ...
  // Init memory
  {
    ...
    // Assign property_material memory
    {
      ...
      data->property_material = reinterpret_cast<Material_id*>(aligned);
      #ifndef NDEBUG
...
material_data_exists(const Material_data *data, const util::generic_id key, size_t *out_index)
{
  ...
  found = util::generic_id_search_linearly(out_index, key, data->material_id, data->size);
...
bool
material_data_get_property_material(const Material_data *data, const util::generic_id key, Material_id *out_value)
{
  size_t index;
