...
bool
mesh_data_push_back(Mesh_data *data, const util::generic_id key, size_t *out_index)
{
  assert(data && key);
...
mesh_data_exists(const Mesh_data *data, const util::generic_id key, size_t *out_index)
{
  ...
  found = util::generic_id_search_linearly(out_index, key, data->mesh_id, data->size);
