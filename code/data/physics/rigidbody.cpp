...
rigidbody_apply_local_force(Rigidbody *rb, const float force[3])
{
  ...
  bt_rb->activate(true);
  bt_rb->applyForce(world_force, rel_pos);
}
...
rigidbody_apply_local_torque(Rigidbody *rb, const float torque[3])
{
  ...
  const btVector3 l_torque = btVector3(torque[0], torque[1], torque[2]);
  const btVector3 w_torque = bt_rb->getWorldTransform().getBasis() * l_torque;

  bt_rb->activate(true);
  bt_rb->applyTorque(w_torque);
}
