...
rigidbody_insert(Rigidbody_data *data, const uint32_t key, const size_t insert_point)
{
  ...
  // Shuffle memory up
  {
    ...
    memmove(&data->field_transform[insert_point * 1], &data->field_transform[start_index * 1], size_to_end * sizeof(*data->field_transform) * 1);
    memmove(&data->field_aabb_collider[insert_point * 1], &data->field_aabb_collider[start_index * 1], size_to_end * sizeof(*data->field_aabb_collider) * 1);
    memmove(&data->field_transformed_aabb_collider[insert_point * 1], &data->field_transformed_aabb_collider[start_index * 1], size_to_end * sizeof(*data->field_transformed_aabb_collider) * 1);
