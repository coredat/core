...
trigger_create(Trigger_data *data, const size_t size_hint)
{
  ...
  bool all_alloc = true;
  {
    ...
    if(all_alloc)
    {
      data->field_trigger = new Bullet_data::Trigger[size_hint * 1];
      assert(data->field_trigger);
      if(!data->field_trigger) { all_alloc = false; }
      else { memset(data->field_trigger, 0, sizeof(Bullet_data::Trigger) * size_hint * 1); }
    }
  }
...
trigger_insert(Trigger_data *data, const uint32_t key, const size_t insert_point)
{
  ...
  // Shuffle memory up
  {
    ...
    memmove(&data->field_trigger[insert_index * 1], &data->field_trigger[insert_point * 1], size_to_end * sizeof(*data->field_trigger) * 1);
...
trigger_resize_capacity(Trigger_data *data, const size_t size_hint)
{
  ...
  // Copy over data
  {
    ...
    memcpy(new_data.field_trigger, data->field_trigger, sizeof(Bullet_data::Trigger) * data->size * 1);
  }
  ...
  // Swap ptrs
  {
    ...
    Bullet_data::Trigger *old_trigger = data->field_trigger;
