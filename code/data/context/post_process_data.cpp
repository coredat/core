
/*
  Warning
  --
  This file is auto generated changes here may be overwritten.
*/


#include "post_process_data.hpp"
#include <assert.h>
#include <cstring>


namespace Data {


// ====================== //
// COMMON MODEL FUNCTIONS //
// ====================== //

namespace {

// Key our keys unique.
uint32_t post_process_model_key_instance_number{0};

} // anon ns


void
data_lock(const Post_process_data *data)
{
  // Not yet impl.
}


void
data_unlock(const Post_process_data *data)
{
  // Not yet impl.
}


bool
post_process_create(Post_process_data *data, const size_t size_hint)
{
  assert(data);
  assert(size_hint);

  // Size up the capacity
  {
    size_t *capacity = const_cast<size_t*>(&data->capacity);
    *capacity = size_hint;
  }

  // Allocate memory
  bool all_alloc = true;
  {
    // Alloc keys
    if(all_alloc)
    {
      data->keys = new uint32_t[size_hint];
      assert(data->keys);
      if(!data->keys) { all_alloc = false; }
      else { memset(data->keys, 0, sizeof(uint32_t) * size_hint); }
    }

    // Alloc space for name
    if(all_alloc)
    {
      data->field_name = new char[size_hint * 32];
      assert(data->field_name);
      if(!data->field_name) { all_alloc = false; }
      else { memset(data->field_name, 0, sizeof(char) * size_hint * 32); }
    }

    // Alloc space for post_shader
    if(all_alloc)
    {
      data->field_post_shader = new Post_renderer::Post_shader[size_hint * 1];
      assert(data->field_post_shader);
      if(!data->field_post_shader) { all_alloc = false; }
      else { memset(data->field_post_shader, 0, sizeof(Post_renderer::Post_shader) * size_hint * 1); }
    }
  }

  // Failed so clean up.
  if(!all_alloc)
  {
    post_process_destroy(data);
  }

  return all_alloc;
}


void
post_process_destroy(Post_process_data *data)
{
  // Free up the memory.
  {
    // Remove keys
    if(data->keys) { delete[] data->keys; }
    data->keys = nullptr;

    // Remove name
    if(data->field_name) { delete[] data->field_name; }
    data->field_name = nullptr;

    // Remove post_shader
    if(data->field_post_shader) { delete[] data->field_post_shader; }
    data->field_post_shader = nullptr;
  }

  // Zero capacity and size
  {
    data->size = 0;

    size_t *capacity = const_cast<size_t*>(&data->capacity);
    *capacity = 0;
  }
}


uint32_t
post_process_push(Post_process_data *data)
{
  assert(data);
  assert(data->keys);

  // Do we need to resize?
  if(data->size >= data->capacity)
  {
    post_process_resize_capacity(data, data->capacity << 1);
  }

  // Push key at the back
  {
    const uint32_t new_key = ++post_process_model_key_instance_number;
    data->keys[data->size++] = new_key;

    return new_key;
  }


  return 0;
}



bool
post_process_remove(Post_process_data *data, const uint32_t key)
{
  size_t index_to_erase = 0;

  if(post_process_exists(data, key, &index_to_erase))
  {
    assert(index_to_erase < data->size);

    const size_t start_index = index_to_erase + 1;
    const size_t size_to_end = data->size - index_to_erase - 1;

    --(data->size);

    // Shuffle the data down
    memmove(&data->keys[index_to_erase], &data->keys[start_index], size_to_end * sizeof(*data->keys));
    memmove(&data->field_name[index_to_erase * 32], &data->field_name[start_index * 32], size_to_end * sizeof(*data->field_name) * 32);
    memmove(&data->field_post_shader[index_to_erase * 1], &data->field_post_shader[start_index * 1], size_to_end * sizeof(*data->field_post_shader) * 1);

    return true;
  }

  return false;
}


bool
post_process_exists(const Post_process_data *data, const uint32_t key, size_t *out_index)
{
  assert(data);
  assert(data != 0);

  for(size_t i = 0; i < data->size; ++i)
  {
    if(data->keys[i] == key)
    {
      if(out_index)
      {
        *out_index = i;
      }

      return true;
    }
  }

  return false;
}


void
post_process_clear(Post_process_data *data)
{
  assert(data);

  data->size = 0;
}


bool
post_process_is_empty(const Post_process_data *data, const size_t size_hint)
{
  assert(data);

  if(!data) { return false; }

  return !!data->size;
}


size_t
post_process_get_size(const Post_process_data *data)
{
  assert(data);

  if(!data) { return 0; }

  return data->size;
}


size_t
post_process_get_capacity(const Post_process_data *data)
{
  assert(data);

  if(!data) { return 0; }

  return data->capacity;
}


bool
post_process_resize_capacity(Post_process_data *data, const size_t size_hint)
{
  assert(data);
  assert(size_hint > data->size); // Will slice data

  // Create new data.
  Post_process_data new_data;
  const bool created_new = post_process_create(&new_data, size_hint);

  // Failed to resize.
  if(!created_new)
  {
    post_process_destroy(&new_data);
    return false;
  }

  // Copy over data
  {
    memcpy(new_data.keys, data->keys, sizeof(uint32_t) * data->size);
    memcpy(new_data.field_name, data->field_name, sizeof(char) * data->size * 32);
    memcpy(new_data.field_post_shader, data->field_post_shader, sizeof(Post_renderer::Post_shader) * data->size * 1);
  }

  // Swap ptrs
  {
    uint32_t *old_keys = data->keys;
    data->keys = new_data.keys;
    new_data.keys = old_keys;

    char *old_name = data->field_name;
    data->field_name = new_data.field_name;
    new_data.field_name = old_name;

    Post_renderer::Post_shader *old_post_shader = data->field_post_shader;
    data->field_post_shader = new_data.field_post_shader;
    new_data.field_post_shader = old_post_shader;
  }

  // Set the Capacity
  {
    size_t *capacity = const_cast<size_t*>(&data->capacity);
    *capacity = new_data.capacity;
  }

  // Destroy new data
  post_process_destroy(&new_data);

  return true;
}



// ===================== //
// DATA GETTER FUNCTIONS //
// ===================== //


const char*
post_process_get_const_name_data(const Post_process_data *data)
{
  assert(data);
  assert(data->field_name);

  return data->field_name;
}


char*
post_process_get_name_data(Post_process_data *data)
{
  assert(data);
  assert(data->field_name);

  return data->field_name;
}


const Post_renderer::Post_shader*
post_process_get_const_post_shader_data(const Post_process_data *data)
{
  assert(data);
  assert(data->field_post_shader);

  return data->field_post_shader;
}


Post_renderer::Post_shader*
post_process_get_post_shader_data(Post_process_data *data)
{
  assert(data);
  assert(data->field_post_shader);

  return data->field_post_shader;
}


// =============== //
// FIELD FUNCTIONS //
// =============== //


bool
post_process_get_name(const Post_process_data *data, const uint32_t key, const char **return_value)
{
  assert(data);
  assert(key != 0);
  assert(data->field_name);
  assert(return_value);

  // Search for its index.
  // If we find it we can return the value.

  size_t index = 0;

  if(post_process_exists(data, key, &index))
  {
    assert(index < data->size);

    if(index < data->size)
    {
      *return_value = &data->field_name[index * 32];

      return true;
    }
  }

  return false;
}


bool
post_process_set_name(const Post_process_data *data, const uint32_t key, const char *set_value, const size_t size)
{
  assert(data);
  assert(key != 0);
  assert(data->field_name);
  assert(set_value);

  // Search for its index.
  // If we find it we can set the value.

  size_t index = 0;

  if(post_process_exists(data, key, &index))
  {
    assert(index < data->size);
    if(index < data->size)
    {
      memcpy(&data->field_name[index * 32], set_value, sizeof(char) * size);

      return true;
    }
  }

  return false;
}


bool
post_process_get_post_shader(const Post_process_data *data, const uint32_t key, Post_renderer::Post_shader *return_value)
{
  assert(data);
  assert(key != 0);
  assert(data->field_post_shader);
  assert(return_value);

  // Search for its index.
  // If we find it we can return the value.

  size_t index = 0;

  if(post_process_exists(data, key, &index))
  {
    assert(index < data->size);

    if(index < data->size)
    {
      *return_value = data->field_post_shader[index];

      return true;
    }
  }

  return false;
}


bool
post_process_set_post_shader(const Post_process_data *data, const uint32_t key, const Post_renderer::Post_shader *set_value)
{
  assert(data);
  assert(key != 0);
  assert(data->field_post_shader);
  assert(set_value);

  // Search for its index.
  // If we find it we can set the value.

  size_t index = 0;

  if(post_process_exists(data, key, &index))
  {
    assert(index < data->size);
    if(index < data->size)
    {
      data->field_post_shader[index] = *set_value;

      return true;
    }
  }

  return false;
}


} // Data ns
